...
function hasConflictingConditions (previousEntity, conditions) {
  ...
  if (previousEntity.synthesis.rules !== undefined) {
    previousEntity.synthesis.rules.forEach((rule) => {
      const previousConditions = rule.conditions || [];
      conflict = conflict || areSameConditions(previousConditions, conditions);
    });
  } else {
    const previousConditions = previousEntity.synthesis.conditions || [];
...
function areSameConditions (condition1, condition2) {
  ...
  const sorted1 = condition1.sort();
  const sorted2 = condition2.sort();

  const sameConditions = sorted1.filter((e1cond, index) => isEqual(e1cond, sorted2[index]));
...
function validateAndRecord (definition, identifier, compositeIdentifier, conditions) {
  const key = identifier || buildKey(compositeIdentifier);
  ...
  if ((
    ...
    const conflictingType = ENTITY.get(key);

    throw new Error(`The rule with identifier ${key} from ${definition.domain}-${definition.type} has same definition than a rule in ${conflictingType.domain}-${conflictingType.type}.`);
  }
  ...
  },
  {
    ...
    apply: def => {
      ...
      if (notAllowed.includes(domainType) && hasSummaryMetrics) {
        throw new Error(`We don't allow custom summary metrics for ${domainType}. Please open an issue if you want to change this type.`);
      }
    }
  },
  {
